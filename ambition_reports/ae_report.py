...
class AEReport(AmbitionCrfPdfReport):
    ...
    def _draw_section_one_header(self, story):
        ...
        story.append(t)
        t = Table(
            [[f"Prepared by {self.get_user(self.ae_initial)}."]], (18 * cm))
            ...
            for obj in qs:
                ...
                t = Table(
                    [
                        [
                            Paragraph(
                                f"{AeFollowup._meta.verbose_name}: {index}", s),
                            Paragraph(username, s),
                            Paragraph(obj.modified.strftime(
                                "%Y-%m-%d %H:%M"), s),
                            Paragraph(
                                fill(self.history_change_message(
                                    obj), width=60), s
                            ),
                        ]
                    ],
